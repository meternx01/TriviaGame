...
var game = {
	...
	fillQuestions: function() {
		...
		this.questions.push({
			...
			answers: ["Feburary 20, 1962", "June 3, 1965", "July 20, 1969", "April 12, 1981"
			],
			correct: 2
		});
		this.questions.push({
		...
		this.questions.push({
			...
			answers: [ "In the Desert", "Splashdown in the Ocean", "Back to the Launch Pad", "Astronauts parachuted out"
			],
			correct: 1
		});
		this.questions.push({
			...
			answers: [ "Richard Nixon", "Lyndon B. Johnson", "Dwight D. Eisenhower", "John F. Kennedy"],
			correct: 3
		});
	},
	...
	answer: function(response) {
		...
		var numQuestions = this.correct + this.incorrect + this.unanswered;
		var imgIndex = numQuestions - 1;
		$("#answerImg").html("<img src='assets/images/" + imgIndex +".jpg' size=240px />");
		this.clearAnswers();
		if (numQuestions < 8) {
			this.writeQuestion(numQuestions);
		} else {
			this.endGame();
		}
	},
	...
	endGame: function() {
		...
		// var bound = game.startGame();
		setTimeout(game.startGame.bind(game), 6000);
